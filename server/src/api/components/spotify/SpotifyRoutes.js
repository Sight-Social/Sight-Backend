...
async function getSpotifyShows(accessToken) {
  ...
  const response = await axios.get(
    `https://api.spotify.com/v1/me/shows?access_token=${accessToken}`
  );
...
async function addShowsToCreatorsCollection(shows, accessToken) {
  ...
  for (const show of shows) {
    ...
    if (!foundCreator) {
      ...
      const newCreatorInsights = await getCreatorInsightsFromSpotify(
        show.show.id,
        accessToken
...
async function getCreatorInsightsFromSpotify(showId, accessToken) {
  ...
  const response = await axios.get(
    `https://api.spotify.com/v1/shows/${showId}/episodes?access_token=${accessToken}&limit=50`
  );
